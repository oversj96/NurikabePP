...
bool Pattern::canMove(int dim, int pos, int dir)
{
    if (dim == 0)
    {
        return pos + dir <= this->rows.size() - 1 && pos + dir >= 0;
    }
    else
    {
        return pos + dir <= this->rowLength - 1 && pos + dir >= 0;
    }
}
...
void Pattern::testPattern(int posX, int posY, int call)
{
    ...
    for (const auto &point : Pattern::points)
    {
        int x = point[0];
        int y = point[1];
        if (this->canMove(0, posX, x) && this->canMove(1, posY, y) && this->travelMap[posX + x][posY + y])
        {
